...
public interface StayReservationDateRepository extends JpaRepository<StayReservedDate, StayReservedDateKey> {
    ...
    // @Query - This annotation is used to define a custom query that goes beyond the standard methods provided by JpaRepository.
    // The query is written in JPQL (Java Persistence Query Language), a language similar to SQL but specific to JPA
    ...
    // Query(value = "SELECT stay_id FROM stay_reserved_date WHERE id IN (:stayIds) date BETWEEN ?1 AND ?2", nativeQuery = true)        // 也可以写成这样。
